...
export class RewardOverlay extends Phaser.GameObjects.Container {
  constructor(
    ...
    const panH = BANNER_H + SUBTITLE_H + 8 + CARD_H + REROLL_SECTION_H + BOTTOM_PAD;
    ...
    banner.fillRoundedRect(px, py, panW, BANNER_H, { tl: 12, tr: 12, bl: 0, br: 0 });
    this.add(banner);

    this.add(scene.add.text(cx, py + BANNER_H / 2, 'CHOOSE YOUR REWARD', {
    ...
    this.add(scene.add.text(cx, py + BANNER_H + 6, 'Select one to keep', {
      fontFamily: THEME.fonts.main,
      fontSize: '11px',
    ...
    const cardsTop = py + BANNER_H + SUBTITLE_H + 8;
    ...
    rewards.forEach((reward, i) => {
      ...
      // Card fill (rectangle for easy hover color swap)
      ...
      hitRect.on('pointerover', () => {
        cardFill.setFillStyle(0x26263a);
      });
      hitRect.on('pointerout', () => {
        cardFill.setFillStyle(0x1a1a2c);
      });
    ...
    if (onReroll !== null) {
      ...
      const rerollBtn = new Button(scene, {
        x: cx,
        y: py + BANNER_H + SUBTITLE_H + 8 + CARD_H + REROLL_SECTION_H / 2,
        width: 220,
        height: 36,
